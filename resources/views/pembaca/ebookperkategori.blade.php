...
<div class="container py-5">
    <h2 class="text-center">Daftar Kategori E-Book {{ $kategori->kategori }}</h2>
    <div class="row row-cols-1 row-cols-md-3 g-4">
        @foreach($ebook as $eperk)
        @if($eperk->id_kategori)
        <div class="col col-sm-3">
            <div class="card">
                <img src="{{ asset('uploads/covers/'.$eperk->cover) }}" class="card-img foto" alt="...">
                <div class="card-body">
                    <h5 class="card-title">{{ $eperk->judul }}</h5>
                    <p class="card-text">{{ Str::limit($eperk->deskripsi,100) }}</p>
                    <div class="d-flex justify-content-around mb-3">
                        <div class="icon-container">
                            ...
                            <span id="icon-text-{{ $eperk->id }}" class="icon-text" style="margin-left: 5px">{{
                                $eperk->jumlah_baca
                                }}</span>
                        </div>
                        <div class="icon-container star-penilaian ">
                            ...
                            <span class="icon-text">({{ number_format($eperk->average_rating, 1) }})</span>
                        </div>
                    </div>
                </div>
                <div class="d-flex justify-content-around mb-5">
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#ebookModal"
                        onclick="baca('{{ $eperk->id }}', '{{ $eperk->judul }}'); jumlahBaca('{{ $eperk->id }}');">Baca</button>
                    <a href="{{ route('detailebook', Crypt::encrypt($eperk->id)) }}"><button
